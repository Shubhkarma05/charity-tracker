...
contract CharityTracker {
    ...
    event DonationRecorded(address indexed donor, string campaignId, uint256 amount, uint256 timestamp, string purpose);
    event ExpenseAdded(string campaignId, uint256 amount, uint256 timestamp, string purpose);
    event CampaignStatusUpdated(string campaignId, string status, uint256 timestamp);
    ...
    function updateCampaignStatus(string memory _campaignId, string memory _status) public {
        require(msg.sender == campaigns[_campaignId].organizer, "Only organizer can update status");
        emit CampaignStatusUpdated(_campaignId, _status, block.timestamp);
    }

    // Function to refund donors
    function refundDonors(string memory _campaignId) public {
        require(block.timestamp > campaigns[_campaignId].endTime, "Campaign is still active");
        require(campaigns[_campaignId].raisedAmount < campaigns[_campaignId].goalAmount, "Goal was met, no refunds");

        Donation[] memory donations = campaignDonations[_campaignId];
        for (uint256 i = 0; i < donations.length; i++) {
            refunds[donations[i].donor] += donations[i].amount;
        }

        campaigns[_campaignId].raisedAmount = 0;
    }

    // Function for donors to claim a refund
